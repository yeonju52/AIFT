...
if __name__ == "__main__":
    ...
    kiwoom.upperBound = (datetime.today() - timedelta(days=1)).strftime('%Y%m%d') + '150000' # 분봉, 일봉마다 다름
    ...
    for i in range(len(st_code)):   # 없으면 IndexError: list index out of range
        ...
        # 새로운 데이터가 있는 경우 upperBound와 같거나 적을 때, kiwoom.lowerBound >= kiwoom.upperBound 역전될 때는 명령문 실행x
        if kiwoom.lowerBound < kiwoom.upperBound:  # 새로 업데이트 된 데이터가 없음: 키움 분봉 조회 실행 X
            df = retrieveTR.TR_min(kiwoom, st_code[i], 60)  # 60분봉으로 변경
            sql.save_query(st_code[i], kiwoom.get_master_code_name(st_code[i]), df) # sql로 저장